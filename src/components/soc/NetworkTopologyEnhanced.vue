<template>
  <DashboardCard title="Network Topology" subtitle="Real-time network connections and threat visualization" size="large">
    <template #header-action>
      <div class="flex gap-2">
        <button
          ...
          :title="autoLayout ? 'Stop auto layout' : 'Start auto layout'"
        >
          <i :class="autoLayout ? 'fas fa-pause' : 'fas fa-play'" class="mr-1"></i>
          {{ autoLayout ? 'Pause' : 'Play' }}
        </button>
        <button
          @click="resetZoom"
...
const buildTopology = () => {
  ...
  logStore.logs.forEach(log => {
    ...
    if (!nodeMap[src]) {
      nodeMap[src] = { x: 0, y: 0, connections: 0, severity: 'Low' }
    }
    if (!nodeMap[dst]) {
      nodeMap[dst] = { x: 0, y: 0, connections: 0, severity: 'Low' }
    }
