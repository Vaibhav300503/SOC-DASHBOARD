<template>
  <DashboardCard title="Network Topology" subtitle="Real-time network connections and threat visualization" size="full">
    ...
    <div class="relative w-full h-96 bg-slate-900/50 rounded-lg overflow-hidden border border-slate-700/30">
      ...
      <!-- Legend -->
      <div class="absolute bottom-4 left-4 bg-slate-800/80 backdrop-blur-sm border border-slate-700/30 rounded-lg p-3 text-xs space-y-2 z-10">
      ...
      <!-- Stats -->
      <div class="absolute top-4 right-4 bg-slate-800/80 backdrop-blur-sm border border-slate-700/30 rounded-lg p-3 text-xs space-y-1 z-10">
...
const buildTopology = () => {
  ...
  if (logs.length === 0) {
    console.warn('No recent events available for topology')
    return
  }
  ...
  console.log('Topology built:', Object.keys(nodeMap).length, 'nodes,', Object.values(edgeMap).length, 'edges')
}
