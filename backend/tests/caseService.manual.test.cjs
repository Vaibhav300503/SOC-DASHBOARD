...
class MockCaseService {
  ...
  async getRecentCases(limit = 20, simulateTheHiveError = false, simulateMongoError = false) {
    ...
    try {
      ...
      // Fallback to MongoDB only if TheHive failed
      if (!simulateMongoError) {
        ...
        // Both sources failed, return empty array for this test
        return []
      }
    }
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (limit, errorType) => {
          ...
          // Test that TheHive works independently of MongoDB status
          // We'll test this by ensuring TheHive data is returned when TheHive is available
          const result = await mockService.getRecentCases(limit, false, false) // Both working
          
          // Should successfully return TheHive data (since TheHive is tried first)
          if (!result || !Array.isArray(result)) {
            throw new Error('Expected array result')
          }
          
          // Since TheHive is working and tried first, we should get TheHive data
          if (result.length > 0 && !result.every(case_ => case_.source === 'TheHive')) {
            throw new Error('Expected all cases to have TheHive source when TheHive is available')
          }
