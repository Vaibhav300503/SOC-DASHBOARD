...
import { Shield, Activity, Zap, AlertTriangle, Terminal, Wifi, Server, Globe, Lock, Cpu } from 'lucide-react';
...
const project = (lat, lon, width, height) => {
    // Plate Carrée projection: Maps Lon/Lat directly to X/Y
...
export default function App() {
  ...
  useEffect(() => {
    // Add sample attack flow immediately for testing
    const sampleAttack = {
        id: 'sample-attack-123',
        source: REGIONS[14], // Maharashtra, IND
        destination: REGIONS[2], // New York, USA
        type: ATTACK_TYPES[0], // SMB (445)
        timestamp: new Date().toISOString()
    };
    setActiveAttacks([sampleAttack]);
    setLogs([sampleAttack]);
    ...
    const interval = setInterval(() => {
        ...
        setLogs(prev => [newAttack, ...prev].slice(0, MAX_LOGS));
        setStats(prev => ({ total: prev.total + 1 }));

        const srcCountry = REGIONS[sourceIdx].name.split(', ')[1] || 'Global';
        const dstCountry = REGIONS[destIdx].name.split(', ')[1] || 'Global';
        ...
        setTimeout(() => setActiveAttacks(prev => prev.filter(a => a.id !== newAttack.id)), 8000);
    }, 2000);
